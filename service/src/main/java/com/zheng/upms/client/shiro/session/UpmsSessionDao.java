...
import com.zheng.common.constants.upms.UpmsConstant;
...
public class UpmsSessionDao extends CachingSessionDAO {
    ...
    protected void doDelete(Session session) {
        ...
        if ("client".equals(upmsType)) {
            ...
            RedisUtil.del(ZHENG_UPMS_CLIENT_SESSION_ID + "_" + sessionId);
            RedisUtil.srem(ZHENG_UPMS_CLIENT_SESSION_IDS + "_" + code, sessionId);
        ...
        if ("server".equals(upmsType)) {
            ...
            // 清除所有局部会话
            Cache jedis = RedisUtil.getJedis();
            Set<String> clientSessionIds = RedisUtil.smembers(ZHENG_UPMS_CLIENT_SESSION_IDS + "_" + code);
            for (String clientSessionId : clientSessionIds) {
                RedisUtil.del(ZHENG_UPMS_CLIENT_SESSION_ID + "_" + clientSessionId);
                RedisUtil.srem(ZHENG_UPMS_CLIENT_SESSION_IDS + "_" + code, clientSessionId);
            }
            LOGGER.debug("当前code={}，对应的注册系统个数：{}个", code, RedisUtil.scard(ZHENG_UPMS_CLIENT_SESSION_IDS + "_" + code));
    ...
    public Map getActiveSessions(int offset, int limit) {
        ...
        // 获取在线会话总数
        long total = RedisUtil.llen(ZHENG_UPMS_SERVER_SESSION_IDS);
        // 获取当前页会话详情
        List<String> ids = RedisUtil.lrange(ZHENG_UPMS_SERVER_SESSION_IDS, offset, (offset + limit - 1));
