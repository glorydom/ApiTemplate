package com.dto.huiyi.meeting.entity.participantDto;

import java.io.Serializable;
import java.util.Date;

public class ParticipantCZHdto implements Serializable {

    private static final long serialVersionUID = 1L;

    private String Status;

    private Integer ID;
    private String NO;
    private String L_NO;
    private String COMPANY;
    private String COMPANY_EN;
    private String PERSON;
    private String PERSON_EN;
    private String ZHIWU;
    private String ZHIWU_EN;
    private String TEL;
    private String FAX;
    private String MOBILE;
    private String LXDH;
    private String EMAIL;
    private String SFJB;
    private String SFXC;
    private String XCFF;
    private String CM;
    private String SM;
    private String FM;
    private String SFHY;
    private int CASH;
    private String SFDK;
    private String SFCJWY;
    private String WYZW;
    private String HOTEL;
    private String ORDER_NO;
    private String SFQD;
    private String SFLQZL;
    private String SCZT;
    private Date RE_DATE;
    private String IF_DEL;
    private String NOTE;
    private String CODE;
    private String LW;
    private String PASS;
    private String WXH;
    private String PIC;
    private String YJDZ;
    private String CG;
    private String JDNO;
    private String TJ;

    public String getStatus() {
        return Status;
    }

    public void setStatus(String status) {
        Status = status;
    }

    public Integer getID() {
        return ID;
    }

    public void setID(Integer ID) {
        this.ID = ID;
    }

    public String getNO() {
        return NO;
    }

    public void setNO(String NO) {
        this.NO = NO;
    }

    public String getL_NO() {
        return L_NO;
    }

    public void setL_NO(String l_NO) {
        L_NO = l_NO;
    }

    public String getCOMPANY() {
        return COMPANY;
    }

    public void setCOMPANY(String COMPANY) {
        this.COMPANY = COMPANY;
    }

    public String getCOMPANY_EN() {
        return COMPANY_EN;
    }

    public void setCOMPANY_EN(String COMPANY_EN) {
        this.COMPANY_EN = COMPANY_EN;
    }

    public String getPERSON() {
        return PERSON;
    }

    public void setPERSON(String PERSON) {
        this.PERSON = PERSON;
    }

    public String getPERSON_EN() {
        return PERSON_EN;
    }

    public void setPERSON_EN(String PERSON_EN) {
        this.PERSON_EN = PERSON_EN;
    }

    public String getZHIWU() {
        return ZHIWU;
    }

    public void setZHIWU(String ZHIWU) {
        this.ZHIWU = ZHIWU;
    }

    public String getZHIWU_EN() {
        return ZHIWU_EN;
    }

    public void setZHIWU_EN(String ZHIWU_EN) {
        this.ZHIWU_EN = ZHIWU_EN;
    }

    public String getTEL() {
        return TEL;
    }

    public void setTEL(String TEL) {
        this.TEL = TEL;
    }

    public String getFAX() {
        return FAX;
    }

    public void setFAX(String FAX) {
        this.FAX = FAX;
    }

    public String getMOBILE() {
        return MOBILE;
    }

    public void setMOBILE(String MOBILE) {
        this.MOBILE = MOBILE;
    }

    public String getLXDH() {
        return LXDH;
    }

    public void setLXDH(String LXDH) {
        this.LXDH = LXDH;
    }

    public String getEMAIL() {
        return EMAIL;
    }

    public void setEMAIL(String EMAIL) {
        this.EMAIL = EMAIL;
    }

    public String getSFJB() {
        return SFJB;
    }

    public void setSFJB(String SFJB) {
        this.SFJB = SFJB;
    }

    public String getSFXC() {
        return SFXC;
    }

    public void setSFXC(String SFXC) {
        this.SFXC = SFXC;
    }

    public String getXCFF() {
        return XCFF;
    }

    public void setXCFF(String XCFF) {
        this.XCFF = XCFF;
    }

    public String getCM() {
        return CM;
    }

    public void setCM(String CM) {
        this.CM = CM;
    }

    public String getSM() {
        return SM;
    }

    public void setSM(String SM) {
        this.SM = SM;
    }

    public String getFM() {
        return FM;
    }

    public void setFM(String FM) {
        this.FM = FM;
    }

    public String getSFHY() {
        return SFHY;
    }

    public void setSFHY(String SFHY) {
        this.SFHY = SFHY;
    }

    public int getCASH() {
        return CASH;
    }

    public void setCASH(int CASH) {
        this.CASH = CASH;
    }

    public String getSFDK() {
        return SFDK;
    }

    public void setSFDK(String SFDK) {
        this.SFDK = SFDK;
    }

    public String getSFCJWY() {
        return SFCJWY;
    }

    public void setSFCJWY(String SFCJWY) {
        this.SFCJWY = SFCJWY;
    }

    public String getWYZW() {
        return WYZW;
    }

    public void setWYZW(String WYZW) {
        this.WYZW = WYZW;
    }

    public String getHOTEL() {
        return HOTEL;
    }

    public void setHOTEL(String HOTEL) {
        this.HOTEL = HOTEL;
    }

    public String getORDER_NO() {
        return ORDER_NO;
    }

    public void setORDER_NO(String ORDER_NO) {
        this.ORDER_NO = ORDER_NO;
    }

    public String getSFQD() {
        return SFQD;
    }

    public void setSFQD(String SFQD) {
        this.SFQD = SFQD;
    }

    public String getSFLQZL() {
        return SFLQZL;
    }

    public void setSFLQZL(String SFLQZL) {
        this.SFLQZL = SFLQZL;
    }

    public String getSCZT() {
        return SCZT;
    }

    public void setSCZT(String SCZT) {
        this.SCZT = SCZT;
    }

    public Date getRE_DATE() {
        return RE_DATE;
    }

    public void setRE_DATE(Date RE_DATE) {
        this.RE_DATE = RE_DATE;
    }

    public String getIF_DEL() {
        return IF_DEL;
    }

    public void setIF_DEL(String IF_DEL) {
        this.IF_DEL = IF_DEL;
    }

    public String getNOTE() {
        return NOTE;
    }

    public void setNOTE(String NOTE) {
        this.NOTE = NOTE;
    }

    public String getCODE() {
        return CODE;
    }

    public void setCODE(String CODE) {
        this.CODE = CODE;
    }

    public String getLW() {
        return LW;
    }

    public void setLW(String LW) {
        this.LW = LW;
    }

    public String getPASS() {
        return PASS;
    }

    public void setPASS(String PASS) {
        this.PASS = PASS;
    }

    public String getWXH() {
        return WXH;
    }

    public void setWXH(String WXH) {
        this.WXH = WXH;
    }

    public String getPIC() {
        return PIC;
    }

    public void setPIC(String PIC) {
        this.PIC = PIC;
    }

    public String getYJDZ() {
        return YJDZ;
    }

    public void setYJDZ(String YJDZ) {
        this.YJDZ = YJDZ;
    }

    public String getCG() {
        return CG;
    }

    public void setCG(String CG) {
        this.CG = CG;
    }

    public String getJDNO() {
        return JDNO;
    }

    public void setJDNO(String JDNO) {
        this.JDNO = JDNO;
    }

    public String getTJ() {
        return TJ;
    }

    public void setTJ(String TJ) {
        this.TJ = TJ;
    }
}
